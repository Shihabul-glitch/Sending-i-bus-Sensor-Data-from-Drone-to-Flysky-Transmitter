...
void IBusBM::begin(HardwareSerial &serial, int8_t timerid, int8_t rxPin, int8_t txPin) {
  ...
  if (!IBusBMfirst && timerid != IBUSBM_NOTIMER) {
      ...
      // on other architectures we need to use a time
        ...
        // NEW (3.x API) – works on ESP32 core 3.x
        timer = timerBegin(1000000);                 // 1 MHz timer tick (1 µs resolution)
        timerAttachInterrupt(timer, &onTimer);       // no 'edge' parameter in 3.x
        timerAlarm(timer, 1000, true, 0);            // 1000 µs = 1 ms, autoreload, unlimited
      #elif defined(_VARIANT_ARDUINO_STM32_)
